...
function App() {
    ...
    useEffect(() => {
        axios.post('https://serverbarbaropersonal.pagekite.me/session/get-session-data', user, {
            ...
            .then(response => {
                login(response.data);
                if (location.pathname === '/login' && location.pathname === '/registration' && location.pathname === '/recovery-password' && location.pathname === '/reset-password' && location.pathname === '/verifie-email') {
                    navigate('/');
                }
            })
            .catch(error => {
